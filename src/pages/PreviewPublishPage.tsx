...
const PreviewPublishPage: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-white rounded-xl border border-[#e9edf2] p-6 mb-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          ...
          <div>
            ...
            <p className="font-medium text-[#0f172a]">{currentTest.subject || '-'}</p>
          </div>
          <div>
            <span className="text-[#94a3b8]">Type</span>
            <p className="font-medium text-[#0f172a] capitalize">{currentTest.type || 'Chapterwise'}</p>
          </div>
          <div>
            <span className="text-[#94a3b8]">Difficulty</span>
            <p className="font-medium text-[#0f172a] capitalize">{currentTest.difficulty || 'Medium'}</p>
          </div>
          <div>
            <span className="text-[#94a3b8]">Questions</span>
            <p className="font-medium text-[#0f172a]">{questions.length} / {currentTest.total_questions || 0}</p>
          </div>
          <div>
            <span className="text-[#94a3b8]">Total Marks</span>
            <p className="font-medium text-[#0f172a]">{currentTest.total_marks || 0}</p>
          </div>
          <div>
            <span className="text-[#94a3b8]">Duration</span>
            <p className="font-medium text-[#0f172a]">{currentTest.total_time || 0} min</p>
          </div>
          <div>
            <span className="text-[#94a3b8]">Marking</span>
            <p className="font-medium text-[#0f172a] text-xs">
              +{currentTest.correct_marks || 5} / -{Math.abs(currentTest.wrong_marks || 1)} / 0
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="mb-6">
        <h3 className="font-semibold text-[#0f172a] mb-3">Questions ({questions.length})</h3>
        {questions.length === 0 ? (
          ...
          <div className="space-y-4">
            {questions.map((q, idx) => (
              <div key={idx} className="bg-white rounded-xl border border-[#e9edf2] p-5">
                <div className="flex items-start gap-3">
                  <span className="font-semibold text-[#4f46e5] text-sm min-w-[28px]">Q{idx + 1}.</span>
                  <div className="flex-1">
                      ...
                      {['option1', 'option2', 'option3', 'option4'].map((opt, i) => (
                        <div
                          ...
                          className={`flex items-center gap-2 text-sm p-2 rounded-lg ${
                            q.correct_option === opt ? 'bg-[#dcfce7] border border-[#86efac]' : 'bg-[#f8fafc]'
                          }`}
                        >
                          <span
                            className={`w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold ${
                              q.correct_option === opt ? 'bg-[#22c55e] text-white' : 'bg-[#e2e8f0] text-[#64748b]'
                            }`}
                          >
                    ...
                    {q.explanation && (
                      <div className="mt-3 text-sm bg-[#fefce8] border border-[#fde68a] rounded-lg p-3">
                        ...
                        <span className="text-[#78350f] ml-1">{q.explanation}</span>
                      </div>
                    )}
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-2">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate(`/edit-test/${id}`)} className="btn-secondary flex items-center gap-2">
            <i className="fas fa-pen"></i> Edit Test
          </button>
          <button onClick={() => navigate(`/test/${id}/questions`)} className="btn-secondary flex items-center gap-2">
        ...
        {!isPublished ? (
          ...
          >
            {publishing ? <span className="spinner w-4 h-4"></span> : <i className="fas fa-rocket"></i>}
